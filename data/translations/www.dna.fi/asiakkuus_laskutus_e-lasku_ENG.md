Directly to content

  1. [ Support and Help ](https://www.dna.fi/tuki)
  2. [ Account ](https://www.dna.fi/tuki/asiakkuus)
  3. [ Billing ](https://www.dna.fi/tuki/asiakkuus/laskutus)
  4. E-Invoice

# E-Invoice

Take advantage of an easier, electronic payment method that also saves the environment.

  * You can conveniently change the invoice payment account in your online bank if needed.
  * Invoices are stored in the archive of your online bank.
  * E-Invoice is a secure invoice reception and payment method developed by banks.

## Switch to E-Invoice like this

You can easily switch to an e-invoice in your online bank, even when you are next paying an invoice.

  * **Speech and mobile broadband subscriptions:** select "DNA Oyj" as the biller and "Mobile Subscriptions, payment deadline and TV" as the billing subject. You will need the reference number from your latest invoice as the identification information.
  * For invoices for **DNA broadband services**, select "DNA Oy" as the biller and "Broadband Subscriptions" as the billing subject. You will need your customer ID and reference number as the identification information.
  * If your invoice includes charges for **DNA Home Phone or DNA Home Line services**, select "DNA Oyj" as the biller and "Home Phone Subscriptions" as the billing subject. You will also need the customer ID and reference number shown on the invoice.

  * You can conveniently change the invoice payment account in your online bank if needed.
  * Invoices are stored in the archive of your online bank.
  * E-Invoice is a secure invoice reception and payment method developed by banks.

## Go to your online bank here

[](https://www.aktia.fi/fi/)

[](http://www.danskebank.fi/fi-
fi/Henkiloasiakkaat/Pages/henkiloasiakkaat.aspx)

[](http://www.handelsbanken.fi/shb/inet/Icentfi.nsf/vlookupfirstpage/handelsbankenfi)

[](http://www.nordea.fi/)

[](https://www.omasp.fi/fi)

[](https://www.op.fi/)

[](https://www.poppankki.fi/)

[](http://www.s-pankki.fi/)

[](https://www.saastopankki.fi/)

[](https://www.alandsbanken.fi/)

Did you find the information you were looking for on this page? Your feedback is important!  593 responses

 __

Yes I found it

118

+1

 __

Partially

77

+1

 __

Not at all

259

+1

 __

A little unclear

139

+1

Hidden

app2.lrdxp-prod.dna.fi

## DNA Chat

## This website uses cookies

We use cookies to ensure you have the best possible experience on our website and services, to track the use of our services and to make advertising meaningful. You can learn about cookies and manage them at any time. [Read the privacy policy.](https://corporate.dna.fi/tietosuoja#evasteet)

Only necessary Accept all

Customize cookie settings

## Cookies in DNA services

A cookie is a small text file that is stored on your device when you visit DNA's digital services or mobile apps. It is a numerical, anonymous identifier that does not harm your device or files, but helps us develop our services.  
  
  
[Read more about DNA's privacy](https://corporate.dna.fi/tietosuoja)

Allow all cookies

### Cookie settings

#### Necessary cookies

Always active

These cookies are necessary for the site to function and cannot be disabled. They are usually set only when you use functions that create service requests, such as setting privacy preferences, logging in, or filling out forms. You can set your browser to block these cookies or warn you about them, but some parts of the site may not work properly afterwards.

Learn more‎

#### Measurement and analytics cookies

Measurement and analytics cookies

Analytics cookies allow us to count visits and traffic sources so we can measure and improve our site's performance. They help us identify popular and less popular pages and how visitors move around the site. All cookie data is collected anonymously. If you do not accept these cookies, we will not know when you have visited our site.

Learn more‎

#### Functional cookies

Functional cookies

Functional cookies allow us to provide better and more personalized features, such as videos and live chat. Cookies may be set by us or by a third party; such as a service provider whose services have been added to our pages. If you do not accept these cookies, some or all of these features may not work properly.

Learn more‎

#### Advertising and social media cookies

Advertising and social media cookies

These cookies are set by our website or services either by our advertising partners or by the social media service provider we use. Through cookies, these companies can build a profile of your interests and show you ads or social media content that you are interested in. This can also affect the content and messages you see on other websites. If you do not accept these cookies, you will not see targeted advertising on different websites and may not be able to use social media sharing tools. Cookies identify the browser and device.

Learn more‎

Back Button

### Cookie list

Search Icon

Filter Icon

Clear

checkbox label label

Apply Cancel

Consent Leg.Interest

checkbox label label

checkbox label label

checkbox label label

Only necessary Confirm my choices

[](https://www.onetrust.com/products/cookie-consent/)